...
contrast_colors <- function(pal, 
  ...
  if (verbose) {
    if (any(ratio_df$low_contrast)) {
      cli::cli_alert_info(c("At least one set of colors failed to meet the minimum contrast threshold of {min_ratio}. Consider adjusting the palette and/or using more distinct label colors."))
      color_info(ratio_df, dark, light)
    }
  }
  if (plot) {
    p <- plot_contrast_colors(ratio_df, dark, light)
    print(p)
  }
...
#' @export
#' @rdname contrast_colors
mutate_contrast <- function(x, col, pal, dark = "black", light = "white", min_ratio = 4.5, verbose = TRUE) {
  if (!inherits(x, "data.frame")) {
    cli::cli_abort("{.arg x} should be a data frame or tibble.")
  }
  # let contrast_colors handle most errors
  col_vct <- dplyr::pull(x, {{ col }})
  ...
  dplyr::left_join(x, cpal, by = join)
}
...
plot_contrast_colors <- function(x, dark, light) {
  ...
  p <- ggplot2::ggplot(x, ggplot2::aes(x = lbl_color, y = forcats::fct_rev(fill)))
