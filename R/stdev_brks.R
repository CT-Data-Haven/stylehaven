...
#' @param filters An optional named list of values to use for filtering. If given, the observation matching these values will be used as the midpoint. If NULL (the default), the midpoint will be calculated as the mean of values, grouped by `x`'s grouping columns (if any) and the arguments to `by` (also if any). Each item in that list should be a vector of length 1.
...
stdev_brks <- function(x,
    ...
    if (!is.null(filters)) {
        ...
        # should only have 1 observation of each filter-groupby combo
        midpt_df <- dplyr::semi_join(x_out, filter_df, by = names(filters))
        midpt_df <- dplyr::select(midpt_df, tidyselect::all_of(c(by, names(filters))), midpt = {{ value }})
        # can drop names(filters) after checking for n combinations
        filter_combos <- dplyr::count(midpt_df, dplyr::across(tidyselect::all_of(c(by, names(filters)))))
        if (any(filter_combos$n > 1)) {
            cli::cli_abort(c(
                "The {.arg filters} and {.arg by} arguments should combine into unique observations to treat as the midpoint.",
                "i" = "Try a more granular combination of {.arg filters} and/or {.arg by}."
            ))
        }
        midpt_df <- dplyr::select(midpt_df, -tidyselect::all_of(names(filters)))
