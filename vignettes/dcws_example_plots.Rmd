...
ggplot(govt_left, aes(x = year, y = value, color = group, group = group)) +
  ...
  scale_color_manual(values = unname(qual_pal[c("pink", "yellow", "lime", "indigo")])) +
...
ggplot(cohesion, aes(x = question, y = value, color = group)) +
  ...
  scale_y_continuous(breaks = NULL) +
  scale_color_manual(values = unname(qual_pal[c("gray", "pink", "lime", "indigo")]),
...
The PNG file this exported:

```{r echo=FALSE}
knitr::include_graphics(file.path(out_dir, "png", "government_trend.png"))
```
