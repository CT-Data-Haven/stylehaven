...
test_that("round_sum100 prints messages", {
  expect_message(round_sum100(c(0.24, 0.61, 0.15) * 100, 0, verbose = TRUE),
                 ":-)", fixed = TRUE)
  expect_message(round_sum100(c(0.24, 0.6, 0.15) * 100, verbose = TRUE),
                 ":-(", fixed = TRUE)
  expect_silent(s2 <- sum(round_sum100(c(9.2124, 40.292, 50.2), 0, verbose = FALSE)))
})

test_that("round_sum100 fails if any NA values", {
  expect_error(round_sum100(c(0.24, 0.61, NA) * 100))
})
